...
public partial class MainWindow : Window
{
    ...
    {
        const string query =
            "INSERT INTO user_feedback (name, small_story, feedback, station) VALUES (@name, @small_story, @feedback, @station)";

        using var cmd = new MySqlCommand(query, conn);
        cmd.Parameters.AddWithValue("@name", feedbackModel.Name ?? "Anonymous");
        cmd.Parameters.AddWithValue("@small_story", feedbackModel.shortstory ?? string.Empty);
    ...
    private void OnShortStoryEntered(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Vul minimaal 1 karakter in voor het bericht", "Fout", MessageBoxButton.OK,
        ...
        {
            MessageBox.Show($"Het bericht mag maximaal {SmallStoryCount} karakters bevatten", "Fout",
    ...
    private void OnFeedbackEntered(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Vul minimaal 1 karakter in voor feedback", "Fout", MessageBoxButton.OK,
                MessageBoxImage.Warning);
            return;
        }
        ...
        {
            MessageBox.Show("$Feedback mag maximaal {FeedbackCount} karakters bevatten", "Fout",
                MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
